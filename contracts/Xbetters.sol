...
contract XBetters is ERC721A, DefaultOperatorFilterer, Ownable {
    ...
    function whitelistAMint(
        ...
        require(
            _numberMinted(msg.sender) + _quantity <= MAX_WLA_MINTS_PER_ADDRESS,
            "Exceeded the address limit"
        );
        require(
        ...
        require(
            msg.value >= (whitelistAMintPrice * _quantity),
    ...
    function whitelistBMint(
        ...
        require(
            _numberMinted(msg.sender) + _quantity <= MAX_WLB_MINTS_PER_ADDRESS,
            "Exceeded the address limit"
        );
        require(
            totalSupply() + _quantity <= MAX_SUPPLY,
        ...
        require(
            msg.value >= (whitelistBMintPrice * _quantity),
            "Not enough ether sent"
        );
        _safeMint(msg.sender, _quantity);
    }
    ...
    function publicMint(uint _quantity) external payable callerIsUSer {
        ...
        require(
            _numberMinted(msg.sender) + _quantity <= MAX_PUB_MINTS_PER_ADDRESS,
            "Exceeded the address limit"
        );
        require(
            totalSupply() + _quantity <= MAX_SUPPLY,
    ...
    function fiatMint(
        ...
        require(phase == Phase.Public, "Public phase is not active");
        require(
            _numberMinted(_account) + _quantity <= MAX_PUB_MINTS_PER_ADDRESS,
            "Exceeded the address limit"
        );
        require(
            totalSupply() + _quantity <= MAX_SUPPLY,
