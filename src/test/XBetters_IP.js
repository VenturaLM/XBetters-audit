...
describe("XBetters IP", function () {

    async function deploy() {
        // Contracts are deployed using the first signer/account by default
        const [owner, account1] = await ethers.getSigners();

        const Contract = await ethers.getContractFactory("XBetters_IP");
    ...
    describe("whitelistAMint(). This function tests a Merkle Proof", function () {
        ...
        const tests = [
            ...
            [1, ["0x343750465941b29921f50a28e0e43050e5e1c2611a3ea8d7fe1001090d5e1436", "0x1ebaa930b8e9130423c183bf38b0564b0103180b7dad301013b18e59880541ae"], 1, "NotWhitelistedInA"], // Invalid `Proof`
        ];
        ...
        for (let i = 0; i < tests.length; i++) {
            if (i == 0) {
                ...
                it(`FIXME: whitelistAMint(${tests[i][0]}, [${tests[i][1]}]) - Expected: ${tests[i][3]}`, async function () {
                    ...
                    await expect(contract.whitelistAMint(tests[i][0], tests[i][1])).not.to.be.revertedWithCustomError(contract, "NotWhitelistedInA");
                ...
                it(`whitelistAMint(${tests[i][0]}, [${tests[i][1]}]) - Expected: ${tests[i][3]}`, async function () {
                    ...
                    // Mint.
                    await expect(contract.whitelistAMint(tests[i][0], tests[i][1])).to.be.revertedWithCustomError(contract, tests[i][3]);
                });
            }
        }
    });
